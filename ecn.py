...
moeities which are at most needing structure extending to second nearest neighbors.
...
def smiles2carbontypes(smiles):
    ...
    for node0 in graph.nodes:
        ...
        if nbors1 == 'COO':
            ...
            if is_anhydride: 
                yield '', 'acid anhydride or ester' # requires 3rd nearest neighbors to discriminate
...
def factory():
#    print(f"didn't find class")
    return 0
class_dct = defaultdict(factory, class_dct)

u = 0 

# ecn appears to be based on functional groups defined not just by carbons but by heteroatoms or even terminal groups 
# add 1 to all cases where a functional group is defined as something other than a carbon atom giving carbon atom classing
# (may be incorrect, articles are paywalled)
